...
import keel.Algorithms.Fuzzy_Rule_Learning.Hybrid.BRB_FURIA.core.*;
 ...
 */
public class BRB_FURIA extends Classifier implements WeightedInstancesHandler {    
     ...
     */
    public BRB_FURIA(parseParameters params) {
	...
	public void execute() {
		...
		try {
			...
			System.out.println("Number of rules:"+m_Ruleset.size()+"\n");
			double featureSum=0.0;
			for(int i =0;i<m_Ruleset.size();i++){
				RipperRule rule =(RipperRule)m_Ruleset.elementAt(i);
				featureSum+=rule.size()/m_Ruleset.size();
			}
			System.out.println("Number of features:"+featureSum+"\n");
			...
			System.out.println("Number of fried rules:"+1.0*friedRulesNum/isWeka.numInstances());
	...
	public double[] distributionForInstance(Instance datum) throws Exception{ 
		...
		double[] first= new double[datum.numClasses()]; 
		for(int c=0;c<datum.numClasses();c++){
			first[c]=1;
		}
		double second=1;
		double third=1;
		...
		for(int i=0; i < m_Ruleset.size(); i++){
			...
			if(rule.covers(datum)){
				friedRulesNum+=1;
				double actWeight=rule.coverageDegree(datum);
				for(int c=0;c<datum.numClasses();c++){
					first[c]*= actWeight * rule.getBeliefConsequent()[c]+1-actWeight*Utils.sum(rule.getBeliefConsequent());
				}
				second*=1-actWeight*Utils.sum(rule.getBeliefConsequent());
				third*=1-actWeight;
			}
				//System.err.println("Cov: "+rule.getConfidence())
		}
		mu=1.0/(Utils.sum(first)-(datum.numClasses()-1)*second);
		if(1-mu*third!=0){
			for (int c=0;c<datum.numClasses();c++){
				rulesCoveringForEachClass[c]=mu*(first[c]-second)/(1-mu*third);
			}
		}
		...
		if (Utils.sum(rulesCoveringForEachClass)==0){
			...
			for(int i=0; i < m_Ruleset.size(); i++){
				...
				if (Utils.max(rule.getBeliefConsequent()) *secondWeight*rule.coverageDegree(datum) >= maxConfidence){
					maxConfidence = Utils.max(rule.getBeliefConsequent())*secondWeight*rule.coverageDegree(datum);
	...
	public String toString() {
		...
		if(true){//m_Debug es lo que habia
			...
			for(int i=0; i<m_Ruleset.size(); i++)
				sb.append(((RipperRule)m_Ruleset.elementAt(i)).toString(m_Class));
		}
		sb.append("\nNumber of Rules : " 
				+ m_Ruleset.size() + "\n");
