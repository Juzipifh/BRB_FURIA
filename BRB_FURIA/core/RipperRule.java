...
public class RipperRule extends Rule{
  ...
  public double[] m_BeliefConsequent;
  ...
  public void setBeliefConsequent(double [] cl){
	m_BeliefConsequent=cl;
  }

  public double[] getBeliefConsequent(){
	return m_BeliefConsequent;
  }
  ...
  public Object copy(){
	...
	copy.setBeliefConsequent(getBeliefConsequent());
  ...
  public String toString(AttributeWeka classAttr) {
	...
	text.append(" => ");
	for(int c=0;c<m_BeliefConsequent.length;c++){
		text.append(classAttr.name() +
		"=" + classAttr.value(c)+" with " +m_BeliefConsequent[c]+" \n");
	}
...
//    for (int i=0; i < instances.numAttributes()-1; i++){
	...
	for (int j = 0; j < m_Antds.size(); j++){
	  ...
	  if (ext.attribute(((Antd)m_Antds.elementAt(j)).att.index()).isNumeric() && ext.numInstances()>0){
		...
		// Bag 1                                
				if (currentAntd.splitPoint < ext.instance(k).value(currentAntd.att.index()) && ext.instance(k).value(currentAntd.att.index()) != ext.instance(k-1).value(currentAntd.att.index())){                                                          
    ...
    m_BeliefConsequent=new double [data.numClasses()];
	double[] acc=new double[data.numClasses()];
	double cov = 0;
	for (int i = 0; i < data.numInstances(); i++){
	  double membershipValue = tempRule.coverageDegree(data.instance(i));
	  cov += membershipValue;
	  for(int j=0;j<data.numClasses();j++){
		if (j == (int)data.instance(i).classValue()){
			acc[j] += membershipValue;
		  }
	  }
	}
	...
	// m-estimate
	for(int j=0;j<data.numClasses();j++){
		m_BeliefConsequent[j]= acc[j]/ (cov);
	}
  }
